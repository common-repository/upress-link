 ...
 *
 * @link              https://www.upress.io
 * @since             1.0.0
 * @package           Upress_Link
 *
 * @wordpress-plugin
 * Plugin Name:       uPress Link
 * Plugin URI:        https://www.upress.io
 * Description:       uPress Link is a companion plugin for the WordPress hosting manager at https://www.upress.io
 * Version:           1.3.1
...
define( 'UPRESS_USER_AGENT', 'uPress-Link/' . UPL_VERSION );
...
class uPress_Link {
	...
	function upl_admin_menu() {
		add_options_page( _x($this->plugin_name, 'admin page title', 'upresslink'), _x($this->plugin_name, 'admin page menu item', 'upresslink'), 'manage_options', $this->plugin_slug, array( $this, 'upl_admin_view' ) );
	}
	...
	function upl_admin_enqueue_scripts( $hook ) {
		...
		wp_localize_script( $this->plugin_slug . '_js', 'upressAjax', array(
				...
				'on' => __( 'ON' ),
				'off' => __( 'OFF' ),
				'requestSuccess' => __( 'Operation completed successfully', 'upresslink' ),
				'requestFailed' => __( 'Operation could not be completed at this time', 'upresslink' ),
			)
		);
	...
	function upl_ajax_database_search_and_replace() {
		...
		$response = array(
			...
			'debug' => $wpdb,
			'time' => $time,
			'success_msg' => sprintf( __( 'Replace completed for the text "%s" which was replaced by "%s". %d tables scanned with %d total rows. Replacement took %f seconds.', 'upresslink' ),
