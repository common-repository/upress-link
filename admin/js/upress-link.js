(function($) {
    ...
    var toggleSwitchSpinner = function(input, val) {
        ...
        if(val === "show" || (!val && !is_shown)) {
            the_switch.fadeOut('fast', function() {
                spinner.fadeIn('fast').addClass('is-active');
            });
            return;
        }
    };
        ...
        $('body').on('lcs-statuschange', '.switch', function() {
            ...
            $.post(ajaxurl, {
                ...
                if(response.status == "fail") {
                    flashErrorMessage(response.data ? response.data.message : upressAjax.requestFailed);
    ...
    var startAjaxSpinner = function(self) {
        ...
        self.css({
            ...
            self.find('.spinner').addClass('is-active').fadeIn('fast');
    ...
    var initSections = function() {
        ...
        $('.upress-section').each(function() {
            var self = $(this);
            var loaders = $('<div class="overlay"><p>'+ self.data('not-available-text') +'</p></div><div class="overlay-loader"><p><i class="spinner is-active"></i></p></div>');
